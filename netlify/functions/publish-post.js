...
exports.handler = async (event) => {
  ...
  const { secret, datei, datum_veroeffentlicht: datumVeroeffentlicht } = payload;
  ...
  if (typeof datumVeroeffentlicht !== 'string' || !DATUM_PATTERN.test(datumVeroeffentlicht)) {
    return errorResponse(400, '"datum_veroeffentlicht" muss im Format JJJJ-MM-TT angegeben werden.');
  }
  ...
  try {
    ...
    const newContent = updateFrontmatter(existing.content.toString('utf8'), {
      ...
      datum_veroeffentlicht: datumVeroeffentlicht,
    });

    const filename = datei.slice(SOURCE_DIR.length + 1);
    const targetPath = `${TARGET_DIR}/${filename}`;

    await moveFile(datei, targetPath, `Mark post as published: ${filename}`, newContent);
    ...
    return errorResponse(500, 'Post konnte nicht als veröffentlicht markiert werden.', error.message);
  }
};
